...
def preprocess_observation_pytorch(
    ...
    *,
    ...
    image_resolution: Sequence[int] = IMAGE_RESOLUTION,
):
    ...
    for key in image_keys:
        ...
        if tuple(image.shape[1:3]) != tuple(image_resolution):
...
def preprocess_observation_pytorch_with_wo_aug(
    observation,
    *,
    train: bool = False,
    ...
    return SimpleProcessedObservation(
        ...
        token_ar_mask=observation.token_ar_mask,
        token_loss_mask=observation.token_loss_mask,
        img_wo_aug=out_images_wo_aug,
    )


def preprocess_observation_pytorch_with_depth(
    observation,
    *,
    train: bool = False,
    image_keys: Sequence[str] = IMAGE_KEYS,
    ...
    for key in image_keys:
        image = observation.images[key]
        depth = observation.extra_data[key+"_depth"][..., None]  # channle last
        ...
        if tuple(image.shape[1:3]) != tuple(image_resolution):
            logger.info(f"Resizing image {key} from {image.shape[1:3]} to {image_resolution}")
            image = image_tools.resize_with_pad_torch(image, *image_resolution)
        if tuple(depth.shape[1:3]) != tuple(image_resolution):
            batch_size, cur_height, cur_width, _ = depth.shape
            ratio = max(cur_width / image_resolution[1], cur_height / image_resolution[0])
            resized_height = int(cur_height / ratio)
            resized_width = int(cur_width / ratio)
            depth = F.interpolate(
                depth.permute(0, 3, 1, 2), size=(resized_height, resized_width), mode='bilinear', align_corners=False
        ...
        if train:
            ...
            if "wrist" not in key:
                ...
                if max_h > 0 and max_w > 0:
                    ...
                    start_w = torch.randint(0, max_w + 1, (1,), device=image.device)
                    image = image[:, start_h : start_h + crop_height, start_w : start_w + crop_width, :]
                    depth = depth[:, start_h : start_h + crop_height, start_w : start_w + crop_width]
                ...
                depth = torch.nn.functional.interpolate(
                    ...
                    mode="bilinear",
                    align_corners=False,
                ).permute(0, 2, 3, 1)  # [B, H, W, 1] -> [B, 1, H, W]
                ...
                if torch.abs(angle) > 0.1:  # Only rotate if angle is significant
                    ...
                    depth = torch.nn.functional.grid_sample(
                        ...
                        padding_mode="zeros",
                        align_corners=False,
                    ).permute(0, 2, 3, 1)  # [B, 1, H, W] -> [B, H, W, 1]
