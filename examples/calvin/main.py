...
class Args:
    ...
    dataset_path: str = os.getenv("CALVIN_DATASET_PATH", "./data/calvin/task_D_D")
    calvin_config_path: str = os.getenv("CALVIN_CONFIG_PATH", "./third_party/calvin/calvin_models/conf")
    eval_sequences_path: str = str(Path(__file__).resolve().parent / "eval_sequences.json")
    ...
    eval_log_dir: str = os.getenv("CALVIN_EVAL_LOG_DIR", "./outputs/calvin_eval")
    ...
    diverse_inst_path: str | None = os.getenv("CALVIN_DIVERSE_INST_PATH")


class CalvinPolicyClient:
    """Wrapper around websocket client with Calvin-specific preprocessing."""
    
    def __init__(self, host: str, port: int, resize_size: int = 224, replan_steps: int = 5):
...
def evaluate_policy_ddp(policy, env, epoch, calvin_conf_path, eval_sequences_path, num_sequences, eval_log_dir=None, debug=False, create_plan_tsne=False, reset=False, diverse_inst=False, diverse_inst_path=None):
    """
    ...
    # val_annotations = OmegaConf.load(conf_dir / "annotations/new_playtable_validation.yaml")
    if diverse_inst:
        if not diverse_inst_path:
            raise ValueError("`diverse_inst` is enabled but `diverse_inst_path` is not set.")
        with open(diverse_inst_path, "r", encoding="utf-8") as f:
...
def main(args: Args):
    ...
    policy = CalvinPolicyClient(args.host, args.port, args.resize_size, args.replan_steps)
    env = make_env(args.dataset_path)

    evaluate_policy_ddp(policy, env, 0, args.calvin_config_path, args.eval_sequences_path, args.num_sequences, args.eval_log_dir, args.debug, args.create_plan_tsne, args.reset, args.diverse_inst, args.diverse_inst_path)
